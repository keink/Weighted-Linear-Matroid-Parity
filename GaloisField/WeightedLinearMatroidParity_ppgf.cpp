...
void Pivoting_around_M(Matrix2 &C, vector<pr> &M, vector<ll> &Ba, vector<ll> &NBa, vector<ll> &Ba_sub, vector<ll> &NBa_sub, vector<bool> &inBase)
{
	...
	Matrix2 alpha = C.sub_matrix(A, B);
	Matrix2 beta = C.sub_matrix(A, NB);
	...
	Matrix2 gamma_ = -gamma * alpha_inverse;
...
ll mate(ll i)
{
	...
	else if(i<pair_vertex_num+nonpair_vertex_num)
	{
		return i;
	}
	else{
		if((pair_vertex_num+nonpair_vertex_num)%2==0){
			if(i%2==0){
				return i+1;
			}
			else{
...
void Blossom(ll v, ll u, Matrix2 &C, Matrix &Q, vector<vector<ll>> &P, Tree &Bl, Tree &Bl2, vector<ll> &K, vector<ll> &order, vector<ll> &Ba, vector<ll> &NBa, vector<ll> &Ba_sub, vector<ll> &NBa_sub, vector<bool> &inBase, vector<Field> &p, vector<ll> &rho, queue<ll> &que, ll N)
{
	if (!overflow)
	{
		Blossom_num++;
...
vector<ll> Search(Matrix2 &C, vector<bool> &inBase, vector<ll> &Ba, vector<ll> &NBa, vector<ll> &Ba_sub, vector<ll> &NBa_sub, Matrix &Q, vector<ll> &K, Tree &Bl, Tree &Bl2, vector<Field> &p, vector<ll> &order, vector<ll> &rho, vector<vector<ll>> &P, ll N)
{
	if (!overflow)
		Search_num++;
	else
	...
	for (ll i = 0; i < Ba.size(); i++)
	{
		for (ll j = 0; j < NBa.size(); j++)
		{
			if (!C.X[i][j].isZero())
			{
				...
				{
					cout << "infeasible" <<NBa[j]<<" "<<Ba[i]<< endl;
					exit(EXIT_FAILURE);
				}
			}
		}
...
bool DualUpdate(Matrix2 &C, Tree &Bl, vector<Field> &p, vector<ll> &Ba, vector<ll> &NBa, vector<ll> &order, vector<ll> &rho, vector<ll> &K, vector<bool> &inBase, Matrix &Q)
{
	if (!overflow)
	{
		DualUpdate_num++;
...
void Augment(vector<ll> &path, Tree &Bl, Matrix2 &C, vector<ll> &Ba, vector<ll> &NBa, vector<ll> &Ba_sub, vector<ll> &NBa_sub, vector<vector<ll>> &P, vector<ll> &K, vector<ll> &order, vector<ll> &rho, vector<bool> &inBase, Matrix &Q, vector<Field> &p)
{
	if (!overflow)
	{
		Augment_num++;
...
vector<ll> matroid_parity(Matrix2 &A, Field w[], Field w_[])
{
	...
	vector<Field> p;
	p.resize(A.col);
	for (ll i = 0; i < pair_vertex_num; i++)
	{
		p[i] = w[i / 2].divideByTwo();
	}

	for (ll i = pair_vertex_num; i < A.col; i++)
	{
		p[i] = w_[i - pair_vertex_num];
	}
	...
	for (ll I = 0; I < row_size.size(); I++)
	{
		for (ll nonzero_idx = 0; nonzero_idx != nonzero_probs.size(); nonzero_idx++)
		{
			...
			for (ll element_idx = 0; element_idx != elements.size(); element_idx++)
			{
				...
				for (ll file_num = 1; file_num <= 400; file_num++)
				{
					...
					if (mode == 10)
					{
						...
						pair<Matrix, pair<vector<ll>, vector<ll>>> p;

						start_for_steiner=clock();

						p = create_matrix_for_steiner(terminals, G);

						end_for_steiner=clock();

						A = p.first;
						vector<ll> wv = p.second.first;
						vector<ll> w_v = p.second.second;

						w = new Field[wv.size()];
						w_ = new Field[w_v.size()];

						for (int i = 0; i < wv.size(); i++)
						{
							w[i] = RationalNumber(wv[i], 1);
						}
						for (int i = 0; i < w_v.size(); i++)
						{
							w_[i] = RationalNumber(w_v[i], 1);
						}
						pair_vertex_num=wv.size()*2;
					...
					vector<ll> opt_Ba = {};
					int opt_prime = -1;
					for (ll prime_idx = 0; prime_idx != prime_n.size(); prime_idx++)
					{
						...
						if (cur_Ba.size() >= A.row)
						{
							...
							for (ll i = 0; i < cur_Ba.size(); i++)
							{
								if (cur_Ba[i] < A.col)
								{
									if (cur_Ba[i] < pair_vertex_num)
									{
										cur_value += w[cur_Ba[i] / 2];
									}
									else
									{
										cur_value += w_[cur_Ba[i] - pair_vertex_num];
										cur_value += w_[cur_Ba[i] - pair_vertex_num];
									}
								}
							}
